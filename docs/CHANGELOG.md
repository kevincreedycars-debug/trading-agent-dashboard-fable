...
- Added two new Backtest / Accuracy dashboard sub-tabs driven by a new checked-in artifact `data/l2l-trade-gate.json` (built by `backtester/scripts/build_l2l_trade_gate.js`, fully downstream of `data/adr-reach-research.json` and the hourly caches; the builder aborts if its recomputed outcomes disagree with the source artifact on any row). **L2L Trade Gate**: hit rates for every Layer 1 call type (leans separated) and Layer 2 signal side at 50/55/60% of ADR20, each shown against the any-day base rate for the same direction with the added edge, a confidence-bucket check across all eight groups, and a trust summary applying the agreed rule (trusted = hit rate at the 55% threshold above 60%; currently 21 of 24 signals pass, with EUR bullish lean, Gold clear bearish, and XAU/USD sell failing). **Directional Call Trust**: close-to-close checker accuracy by call type with leans separated for Layer 1, and for Layer 2 via a predictionId join back to the target asset's checker verdict; same >60% trust rule (currently 10 of 24 pass). Both tabs carry the caveat that the confidence/strength score is at present unreliable for this kind of directional confluence. Smoke test extended to walk both tabs; backtester suite unaffected.
