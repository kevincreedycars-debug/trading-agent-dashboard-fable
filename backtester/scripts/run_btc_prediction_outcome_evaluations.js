...
const DEFAULT_FLAT_THRESHOLD = 1.00;
...
function summarizePrimaryEvaluations(evaluations) {
  ...
  return {
    ...
    realised_direction: first?.market_outcome_direction || null,
...
async function run() {
  ...
  for (const prediction of predictions) {
    ...
    const openPrice = marketValueFromSnapshot("BTCUSD", observation.market_snapshot)
      ?? marketValueFromSeries("BTCUSD", callDate, priceMap);
    ...
    const closePrice = closeDate ? marketValueFromSeries("BTCUSD", closeDate, priceMap) : null;
    ...
    const row = {
      ...
      calibration_notes: finalEvaluation.evaluable
        ? "BTCUSD remains the current live BTC benchmark convention for historical evaluation."
      ...
      evaluation_payload: finalEvaluation,
      evaluation_version: EVALUATION_VERSION,
      agent_direction: prediction.predicted_direction,
      agent_conviction: prediction.predicted_conviction
    };
  ...
  console.log(JSON.stringify({
    ...
    rows_written: evaluationRows.length,
    realised_rows_written: realisedRows.length,
...
if (require.main === module) {
  run().catch((error) => {
    console.error("BTC prediction outcome evaluation run failed.");
